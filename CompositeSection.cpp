	...
	concrete_part_(concrete_part)
{
 	if (!geometry.is_end_beam())
	{
		...
		double a=steel_part_.get_I_section().get_b_uf()/2;
...
{
	double E_st_ = steel_part_.get_I_steel().get_E_s(); // Модуль упругости стали
...
void CompositeSection::compos_sect_height_calc()
{
	...
	double h_st = steel_part_.get_I_section().get_h_st(); // Высота стальной балки
	H_red_ = h_b + h_st;
}
...
void CompositeSection::dist_conc_to_steel_calc()
{
	...
	double Z_f2_st=steel_part_.get_I_section().get_Z_f2_st();
...
{
	double I_st=steel_part_.get_I_section().get_I_st();
	double A_st = steel_part_.get_I_section().get_A_st();
	double A_b=concrete_part_->get_A_b();
	double A_s=concrete_part_->get_rebar().get_A_s();
	double b=concrete_part_->get_rebar().get_b();
...
{
	double Z_f1_st=steel_part_.get_I_section().get_Z_f1_st();
