...
double str_exp(char *str, int *rc) {
  ...
  char *endptr;
  int i_left;
  char str_left[48];  //для чего используется? Почему 48?
  double member_0[N_MEMBER];//Почему не используется MAX_NUM. Ограничение размера считываемой и анализируемой строк отличаются
        ...
        switch (str[i]) {
		  ...
		  if (exp) {
			...
			i_left++;
			 ...
			 str_left[i_left]='\0';
			 member_0[n_memb]=strtod(str_left, &endptr);//преобразует первое значение в строке в число и возвращает его. В переданный указатель записывается адрес следующего числа
             if (*endptr!=NULL && endptr[0]!=' ') {
                *rc=1;
				return 0.0;
             }
